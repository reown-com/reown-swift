...
enum ImportAccount: Codable {
    ...
    init?(input: String) {
        ...
        default:
            switch true {
            case input.starts(with: ImportAccount.appKit):
                let components = input.components(separatedBy: "-")
                guard components.count == 3, let account = Account(components[1]) else {
                    return nil
                }
                self = .appKit(account: account, topic: components[2])
    ...
    var storageId: String {
        ...
        case .custom(let privateKey):
            return privateKey
        case .appKit(let account, let topic):
            return "\(ImportAccount.appKit)-\(account.absoluteString)-\(topic)"
        }
    }
    ...
    func onSign(message: String) -> CacaoSignature {
        ...
        let signature = try! signer.sign(message: message, privateKey: privateKey, type: .eip155(.eip191))
        return signature
    }
